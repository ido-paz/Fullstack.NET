...
namespace ShopWebAPI.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        public string Test()
        {
            string testMessage = "Hello from Test1";
            try
            {
                var cs = _ShopDbContext.Database.GetConnectionString();
                if (_ShopDbContext.Database.CanConnect())
                    return testMessage + ",DB connection is available";
                else
                    throw new Exception($",{cs} DB connection not available");

            }
            catch (Exception e)
            {
                return testMessage + e.Message;
            }
        }
